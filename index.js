...
const ms = require("ms");
class hayp034Bot extends Client {
  constructor(options) {
    super(options);
  ...
  ok = "<a:hayp034_onay:854402420121403402>"
  ...
  async turkishDate(date) {
    ...
    let convert = hayp034(date, { verbose: true })
  ...
  client.on("messageDelete", async deletedMessage => {
    ...
    sc.send(embed);
      ...
      client.on("guildMemberNicknameUpdate", (member, oldNickname, newNickname) => {
        ...
        let hayp034 = member.guild.channels.cache.get(Log.Nick.Log)
