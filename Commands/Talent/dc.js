...
class DoğrulukCesaret extends Command {
    ...
    async run(message, args, perm) {
        ...
        if (!args[0]) return this.client.yolla("Rol verme biçimini belirt ve tekrar dene. \`Örnek kullanım: .dc sorumlu @hayp034/ID - .dc cezalı @hayp034/ID\`", message.author, message.channel)
       ...
       /* if (args[0] == "elit") {
            ...
            if (!user.roles.cache.has("")) {
                await this.client.yolla(`${user} kişisine <@&> rolü verildi.`, message.author, message.channel)
                user.roles.add("")
            } else {
                await this.client.yolla(`${user} kişisine <@&> rolü alındı.`, message.author, message.channel)
                user.roles.remove("")
            }
        }
*/
