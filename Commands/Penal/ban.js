...
class Ban extends Command {
    ...
    async run(message, args, perm) {
       if(!message.member.roles.cache.has(Permissions.Ban.Auth_Roles) && !message.member.hasPermission("VIEW_AUDIT_LOG")) return
        ...
        //let hayp034 = count ? count++ : 1;
        let hayp034 = count
        const embed = new Discord.MessageEmbed()
            .setAuthor(message.author.tag, message.author.displayAvatarURL({ dynamic: true }))
            .setColor("RANDOM")
            .setDescription(`**${user.tag}** kullanıcısı **${message.author.tag}** tarafından başarıyla sunucudan yasaklandı. (Ceza Numarası: \`#${hayp034 + 1}\`)`).setImage(Permissions.Ban.GIF.length > 0 ? Permissions.Ban.GIF : null);
...
Ban Sebebi: \`${reason}\`
            ...
            await user.send(new Discord.MessageEmbed().setColor("RANDOM").setImage(Permissions.Ban.GIF.length > 0 ? Permissions.Ban.GIF : null).setAuthor(message.author.tag, message.author.displayAvatarURL({ dynamic: true })).setDescription(`
                **${message.guild.name}** sunucusundan ${reason} sebebiyle **${message.author.tag}** tarafından yasaklandın. (Ceza Numaran: \`#${hayp034 + 1}\`)
           ` ))
